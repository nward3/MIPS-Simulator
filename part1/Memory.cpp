...
uint8_t* Memory::getMemoryPtr(unsigned addr) {
  unsigned offset;
  if ((addr <= stackBound) && (addr >= stackBase)) {
	offset = addr - stackBase;
	return stackMemory + offset;
  } else if ((addr <= dataBound) && (addr >= dataBase)) {
	offset = addr - dataBase;
	return dataMemory + offset;
  } else if ((addr <= textBound) && (addr >= textBase)) {
	offset = addr - textBase;
	return textMemory + offset;
  }
